...
export default {
  ...
  data() {
    return {
      ...
      positions: [],
      roles: [{label: "manager", value: "manager"}],
      user: {},
  ...
  async created() {
    ...
    // getSystemRoleList({RoleType:'公司级'}).then((items) => {
    //   if (items.length > 0) {
    //     items.forEach((el) => {
  ...
  methods: {
    ...
    edit(record) {
      ...
      this.$nextTick(() => {
        ...
        this.form.setFieldsValue({ 'extraProperties.ValidityDate': this.user['extraProperties']['ValidityDate'] || undefined })
        if (this.user.id) {
    ...
    handleOk() {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          ...
          formObj.roleNames = ['manager']
          formObj.OrgIds = this.organizationUnitList[0].id
          saveUser(formObj)
            .then((res) => {
