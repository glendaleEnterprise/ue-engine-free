<template>
  <div ref="wrap" :class="{ 'mobile-Model':isMobile  }">
    <a-card size="small" :bordered="false" class="card-custom-style">
      <a-space>
        <a-button ghost @click="CreateFlatten" @keyup.prevent @keydown.enter.prevent>新建压平</a-button>
        <a-button ghost v-if="isMobileOver" @click="MobileOver">结束取点</a-button>
      </a-space>
      <a-list size="small" :data-source="flattenList" :locale="{ emptyText: `暂无数据` }" class="viewpoint-list scroll-box">
        <a-list-item slot="renderItem" slot-scope="item,index">
          <a-space class="viewpoint-title">{{ item.name }}</a-space>
          <a-space slot="actions">
            <a-tooltip title="删除">
              <a-icon type="delete" @click="DelFlatten(item, index)" />
            </a-tooltip>
          </a-space>
        </a-list-item>
      </a-list>
    </a-card>
    <a-modal title="新建压平" :width="320" :visible="visible" @ok="SaveFlatten" @cancel="handleCancel" cancel-text="取消"
      ok-text="确定" :getContainer="() => this.$refs.wrap" v-drag :destroyOnClose="true" :maskClosable="false"
      :mask="false">
      <a-form :form="form" :hideRequiredMark="true">
        <a-form-item label="压平名称" :label-col="{ span: 8 }" :wrapper-col="{ span: 15 }">
          <a-input placeholder="请输入压平名称" v-decorator="['title', { rules: [{ required: true, message: '请输入压平名称' }] }]" />
        </a-form-item>
        <a-form-item label="压平高度" :label-col="{ span: 8 }" :wrapper-col="{ span: 15 }">
          <a-input-number placeholder="请输入压平高度"
            v-decorator="['height', { rules: [{ required: true, message: '请输入压平高度' }], initialValue: 10 }]"
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      that.flattenList = store.state.bim.flattenList;
    },
    methods: {
      CreateFlatten() {
        const that = this;
        ...
        that.setPosition = [];
        that.$message.info(that.isMobile ? '请点击拾取压平范围' : '请点击拾取压平范围，右键结束取点');
          ...
          callback: (data) => {
            ...
            api.Public.event("RIGHT_CLICK", function (e) {
              ...
              if (that.setPosition.length > 2) {
                ...
                that.$message.info('请拾取三个及以上点形成压平范围，右键结束取点')
              }
            });
          }
        })
      ...
      SaveFlatten() {
        ...
        that.form.validateFields((err, values) => {
          if (!err) {
            var opt = {
              id: "flatten" + genlabID(6),
              positions: that.setPosition,
              height: values.height,
              name: values.title,
              modelId: that.modelId ? that.modelId : that.projectMessage.modelId,
            }
            that.flattenList.push(opt)
            api.Model.addFlatten(that.modelId ? that.modelId : that.projectMessage.modelId, opt);
            that.setPosition = []
            that.visible = false;
          }
        })
      },
      DelFlatten(item, index) {
        const that = this
        that.flattenList.splice(index, 1)
    ...
    destroyed() {
      const that = this
      store.dispatch('GetFlattenList', that.flattenList)
    }
  }
</script>
