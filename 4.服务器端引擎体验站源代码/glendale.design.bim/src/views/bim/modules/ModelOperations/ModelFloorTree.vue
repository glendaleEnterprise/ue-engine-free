    ...
    methods: {
      onLoadData(treeNode) {
        ...
        return new Promise((resolve) => {
          ...
          getModelFloorTree({
            PGlid: treeNode.dataRef.glid == treeNode.dataRef.modelId ? '' : treeNode.dataRef.glid,
      ...
      async onCheckTree(checkedKeys, e) {
        ...
        if (checkNode.glid == '0') {
          ...
          const res = await getModelFloorTreeId({
            glId: e.node.dataRef.glid,
            ModelName: e.node.dataRef.modelName
          })
          if (store.state.bim.currentClick == 'FloorTree') {
