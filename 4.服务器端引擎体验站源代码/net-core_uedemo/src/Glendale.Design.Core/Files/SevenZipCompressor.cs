...
namespace Glendale.Design.Files
{
    /// <summary>
    /// 7z压缩
    /// </summary>
    public class SevenZipCompressor : ISevenZipCompressor
    {
        ...
        /// <param name="zipFile">压缩到...</param>
        /// <param name="rootdir">压缩包内部根文件夹</param>
        public void ZipStream(Dictionary<string, Stream> streams, Dictionary<string, string> files, string zipFile, string rootdir = "")
        {
            using var archive = CreateZipArchive(streams, files, rootdir);
            archive.SaveTo(zipFile, new WriterOptions(CompressionType.Deflate)
            {
                LeaveStreamOpen = true,
                ArchiveEncoding = new ArchiveEncoding()
        ...
        /// <param name="rootdir"></param>
        /// <returns></returns>
        private ZipArchive CreateZipArchive(Dictionary<string, Stream> streams, Dictionary<string, string> files, string rootdir)
        {
            ...
            if (streams.Any())
            {
                ...
                {
                    archive.AddEntry(Path.Combine(rootdir, pair.Key), pair.Value);
                }
                foreach (var pair in files)
                {
                    if (Directory.Exists(pair.Value))
                    {
                        archive.AddAllFromDirectory(pair.Value);
                    }
                    else {
                        archive.AddEntry(Path.Combine(rootdir, pair.Key), pair.Value);
                    }
