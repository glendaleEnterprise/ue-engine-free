<template>
  <a-card title="模型目录" size="small" :bordered="false" style="background: transparent">
    <div class="document-tree">
      <vue-scroll>
        <a-tree v-model="selectedKeys" :treeData="myTreeData" :selectedKeys="selectedKeys" :expandedKeys="expandedKeys"
          :autoExpandParent="true" @expand="onExpand" @select="onSelectTree"></a-tree>
      </vue-scroll>
    </div>
  </a-card>
</template>
    ...
    getTree() {
      ...
      getFolderTrees(that.currProject.id).then((res) => {
        if (res.length > 0) {
          ...
          that.$store.dispatch("GetNewDocTree", res)
        } else {
          that.$emit('ok', [])
        }
      })
...
<style lang="less" scoped> /deep/.ant-card-head {
