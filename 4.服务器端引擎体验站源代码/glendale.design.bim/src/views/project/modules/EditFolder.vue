...
export default {
  ...
  methods: {
    add(pid) {
      this.rootDirectory = pid ? false : true;
      this.modalTitle = '添加目录'
      this.isAdd = true
      this.visible = true
      const parentIds = []
      this.getTreeNodeIds(this.docTree, (data) => data.key === pid, parentIds)
    ...
    async edit(id) {
      ...
      if (id) {
        ...
        this.form = res
        const parentIds = []
        this.getTreeNodeIds(this.docTree, (data) => data.key === res.parentId, parentIds)
        this.documentIds = parentIds
      } else {
    ...
    afterForm() {
      ...
      this.$store.dispatch('GetDocTree', this.currProject.id)
      this.$refs.refForm.resetFields()
      this.$emit('ok', this.rootDirectory)
      this.close()
    },
    handleCancel() {
      this.close()
